...
const App: React.FC = () => {
  ...
  const { data: products, subscribeToAll: subscribeToProducts } = useDatabase<Product>('products');
  const { data: orders, create: createOrder, subscribeToAll: subscribeToOrders } = useDatabase<Order>('orders');
  ...
  // Supabase write helpers used by AdminDashboard
  const setProducts = async (updatedProducts: Product[]) => {
    try {
      // Remove created_at from products to let database handle defaults
      // This prevents sending null/undefined which violates NOT NULL constraint
      const productsToUpsert = updatedProducts.map(({ created_at, ...product }) => product);
      ...
      if (refreshedProducts) {
        // This will trigger useDatabase to update via subscription, but we can also update directly
        // For now, rely on subscription
      }
    } catch (err) {
      console.error('Failed to update products in Supabase:', err);
      throw err;
    }
  };
  ...
  const setOrders = async (updatedOrders: Order[]) => {
    try {
      // Orders have JSONB items, ensuring correct format
      const ordersToUpsert = updatedOrders.map(o => ({
        ...o,
        // Ensure date is ISO
        date: o.date.includes('T') ? o.date : new Date(o.date).toISOString()
      }));
      const { error } = await supabase.from('orders').upsert(ordersToUpsert);
      if (error) console.error('Failed to update orders:', error);
    } catch (err) {
      console.error('Failed to update orders in Supabase:', err);
    }
  };
  ...
  const handleLogout = async () => {
    await logout();
    setCart([]);
    // Favorites are automatically filtered by user_id, no need to clear
  };
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen relative bg-[#f0f0f0] selection:bg-[#A3FF00]">
        <LoadingSplash visible={authLoading} />
        {/* Mobile Menu Overlay */}
        {mobileMenuOpen && (
